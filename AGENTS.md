...
- 双车同构：`US`(我方) 与 `THEM`(对手) 各带 14 路传感器 + 同一套 FSM。计分板 `scoreBoard{us,them}` 共享。
- 改完 CORE 后必须跑：`node sim_selftest.js`（25 个确定性场景）确认没破坏状态机，再 `node sim_dragtest.js` 和 `node build_3d.js` 同步 3D 版。
